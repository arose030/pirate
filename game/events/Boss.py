...
class Boss_fight1 (event.Event):
    ...
    def process (self, world):
        ...
        result = {}
        result["message"] = "the Keeper of Peace is defeated!"
        monsters = []
        if random.randrange(10) == 0:
            monsters.append(combat.Keeper("Ultra Keeper of Peace"))
            monsters[0].speed = 1.2*monsters[0].speed
            monsters[0].health = 2*monsters[0].health
        else:
            monsters.append(combat.Keeper("Keeper of Peace "))
        announce ("You are attacked by a Keeper of Peace")
